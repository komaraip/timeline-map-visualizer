...
export const drawCompositeFrame = (options: CompositeFrameOptions) => {
  ...
  if (useBasemap && basemapCanvas?.width && basemapCanvas.height) {
    drawCover(context, basemapCanvas, width, height);
  } else {
    const minimalFrame = overview
      ? { ...track.frameAt(1), completedPaths: track.movements.map((step) => step.path), activePath: [] }
      : frame;
    drawMinimalMap(context, width, height, minimalFrame, cameraProjector);
  }

  const topShade = context.createLinearGradient(0, 0, 0, height * 0.3);
  topShade.addColorStop(0, "rgba(23,44,36,.78)");
  topShade.addColorStop(1, "rgba(23,44,36,0)");
  context.fillStyle = topShade;
  context.fillRect(0, 0, width, height * 0.32);

  const bottomShade = context.createLinearGradient(0, height * 0.62, 0, height);
  bottomShade.addColorStop(0, "rgba(23,44,36,0)");
  bottomShade.addColorStop(1, "rgba(23,44,36,.9)");
  ...
  if (settings.showJourneyDetails) {
    context.fillStyle = "#f3f0e7";
    context.font = `600 ${Math.round(width * 0.032)}px Georgia, serif`;
    context.fillText(
      overview ? "Journey complete" : frame.currentLabel,
      margin,
      height - margin * 2.15,
      width - margin * 2,
    );
    ...
    context.fillText(
      `${track.totalDistanceKm.toFixed(track.totalDistanceKm < 100 ? 1 : 0)} km mapped | ${track.visits.length} visits`,
      margin,
      height - margin * 1.55,
      width - margin * 2,
    );
  ...
  const progressY = height - margin * 0.72;
  const progressHeight = Math.max(5, height * 0.004);
  context.fillStyle = "rgba(243,240,231,.28)";
  context.fillRect(margin, progressY, width - margin * 2, progressHeight);
  context.fillStyle = "#d8ff5f";
  context.fillRect(margin, progressY, (width - margin * 2) * frame.progress, progressHeight);
  context.font = `700 ${Math.round(width * 0.014)}px Arial, sans-serif`;
  context.fillStyle = "rgba(243,240,231,.7)";
