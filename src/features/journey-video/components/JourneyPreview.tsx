  ...
  return (
    <section className="studio-stage" aria-label="Animated journey preview">
      <div className={`studio-preview studio-${settings.aspectRatio}`}>
        ...
        <div className="studio-title-card">
          <strong>{settings.title || "My journey"}</strong>
          <span>{settings.subtitle || period}</span>
        </div>
        {settings.showJourneyDetails && (
          <div className="studio-now">
            <strong>{progress >= 1 ? "Journey complete" : frame.currentLabel}</strong>
            <span>
              {track.totalDistanceKm.toFixed(track.totalDistanceKm < 100 ? 1 : 0)} km mapped
              {" | "}{track.visits.length} visits
            </span>
          </div>
        )}
      ...
      <div className="studio-transport">
        ...
        <button type="button" className="studio-play" onClick={onToggle} disabled={!track.steps.length}>
          {playing ? "Pause" : "Play preview"}
        </button>
        <input
          aria-label="Journey preview position"
          type="range"
          min="0"
          max="1"
          step="0.001"
      ...
      {!track.movements.length && (
        <p className="studio-notice">
          This selection has no mapped movement. The video will reveal visits without drawing invented routes.
        </p>
      )}
    </section>
  );
