import { distanceKm, pathDistanceKm } from "../timeline/analytics";
import { eventTime, type MovementEvent, type Position, type TimelineEvent, type VisitEvent } from "../timeline/types";
...
export const chooseRecordingMimeType = (isSupported: (mimeType: string) => boolean) => {
  ...
  return candidates.find((candidate) => isSupported(candidate.mimeType)) ?? { mimeType: "", extension: "webm" as const };
};
